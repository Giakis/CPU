...
public class WorstFit extends MemoryAllocationAlgorithm {
    ...
    public int fitProcess(Process p, ArrayList<MemorySlot> currentlyUsedMemorySlots) {
        boolean fit = false;
        int address = -1;
        ArrayList<Integer> NewMemorySlot = new ArrayList<>(currentlyUsedMemorySlots.size()); //a arraylist that is used for find the right place.
        for (int i = 0 ; i < currentlyUsedMemorySlots.size(); i++) {
            ...
            NewMemorySlot.add(i, sizeOfBlock);
        }
        bubbleSortArrayList(NewMemorySlot); //declining
